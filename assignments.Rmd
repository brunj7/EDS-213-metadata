...
students %>% 
  ...
  group_by((row_number()-1) %/% (n()/9)) %>%  # create the Groups
...
### Assignment 2 -- Define your group project question
...
students %>% 
  slice(sample(1:n())) %>%   # randomly arrange the data frame
  group_by((row_number()-1) %/% (n()/7)) %>%  # create the Groups
  nest %>% pull() %>% bind_rows(.id = "Group") %>%   # group number as column
  datatable(options = list(pageLength = 25))   # Display
